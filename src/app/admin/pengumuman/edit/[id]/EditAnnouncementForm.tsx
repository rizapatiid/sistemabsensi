...
export default function EditAnnouncementForm({ ann }: { ann: any }) {
  ...
  const insertText = (before: string, after: string) => {
    const textarea = document.getElementById('announcement-content-edit') as HTMLTextAreaElement;
    if (!textarea) return;
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px', marginBottom: '32px' }}>
              <div>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginBottom: '12px' }}>
                      ...
                      <span style={{ fontSize: '0.65rem', fontWeight: 800, color: '#94a3b8', letterSpacing: '0.05em', textTransform: 'uppercase' }}>Broadcasting Center • Update Content</span>
                  </div>
                  ...
                  <p style={{ color: '#64748b', fontWeight: 600, fontSize: 'clamp(0.85rem, 2vw, 1rem)', marginTop: '8px', margin: 0 }}>
                    Lakukan modifikasi pada materi publikasi yang telah disiarkan.
                  </p>
              </div>
          </div>
      </div>
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    <div className={styles.formGroup}>
                        ...
                        <div className={styles.searchBox} style={{ background: '#f8fafc', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                            ...
                            <input style={{ background: 'transparent', border: 'none', fontWeight: 700 }} className={styles.searchInput} type="text" name="judul" defaultValue={ann.judul} required />
                        </div>
                    </div>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 280px), 1fr))', gap: '24px' }}>
                        <div className={styles.formGroup}>
                            <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>Perbarui Banner (Opsional)</label>
                        ...
                        <div className={styles.formGroup}>
                            <label style={{ display: 'block', fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '12px' }}>Jadwal Penayangan</label>
                            <div className={styles.searchBox} style={{ background: '#f8fafc', borderRadius: '14px', border: '1px solid #e2e8f0' }}>
                                <div className={styles.searchIcon}><IconClock /></div>
                                <input 
                                    style={{ background: 'transparent', border: 'none', fontWeight: 700, appearance: 'auto' }} 
                            ...
                            <p style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, marginTop: '8px' }}>*Ganti atau hapus jadwal untuk tayang segera.</p>
                        </div>
                    </div>

                    <div className={styles.formGroup}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '12px' }}>
                            <label style={{ fontSize: '0.7rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Isi Pengumuman Lengkap</label>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            <textarea 
                                id="announcement-content-edit"
                                name="konten" 
                                rows={10} 
                                defaultValue={ann.konten}
                                placeholder="Revisi atau perbarui isi pengumuman di sini..." 
                                required 
                                style={{ width: "100%", padding: "16px 20px 16px 50px", borderRadius: "16px", background: '#f8fafc', border: '1px solid #e2e8f0', fontWeight: 700, fontSize: '0.85rem', color: '#1e293b', outline: 'none', transition: 'all 0.2s', resize: 'none', boxSizing: 'border-box', lineHeight: '1.6' }}
                            ></textarea>
                        </div>
                        <p style={{ fontSize: '0.65rem', color: '#94a3b8', fontWeight: 700, marginTop: '8px' }}>*Gunakan toolbar untuk revisi gaya teks (Tebal, Miring, List).</p>
                    </div>

                    <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
                        <Link href="/admin/pengumuman" className={styles.btnAction} style={{ flex: 1, background: '#f1f5f9', color: '#64748b', border: '1px solid #e2e8f0', justifyContent: 'center', padding: '16px 12px', borderRadius: '16px', fontWeight: 900, fontSize: '0.85rem', boxShadow: 'none' }}>
                          BATAL
                        </Link>
                        <button type="submit" disabled={loading} style={{ flex: 2, padding: '16px 12px', fontSize: '0.85rem', borderRadius: '16px', fontWeight: 900, border: 'none', color: 'white', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1, background: '#0f172a', transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)', boxShadow: '0 10px 15px -3px rgba(15, 23, 42, 0.1)' }}>
