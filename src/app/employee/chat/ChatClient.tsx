...
import { getChatList, sendMessage, getMessages, findUserById, deleteMessage, deleteConversation, findFirstAdmin } from "@/actions/chat"
...
export default function ChatClient({ user }: { user: any }) {
  ...
  const fetchMessages = async () => {
    ...
    const res = await getMessages(user.id, selectedUser.id)
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const res = await sendMessage(user.id, inputText, selectedUser.id)
    if (res.success) {
      setInputText("")
      fetchMessages()
      fetchChatList()
  ...
  const handleStartAdminChat = async () => {
    const res = await findFirstAdmin()
    if (res.success && res.admin) {
      if (!users.find(u => u.id === res.admin?.id)) {
        setUsers(prev => [res.admin, ...prev])
      }
      setSelectedUser(res.admin)
      setIsModalOpen(false)
    } else {
  ...
  const executeDelete = async () => {
    ...
    if (confirmData.type === 'message' && confirmData.id) {
      const res = await deleteMessage(confirmData.id, user.id)
      if (res.success) fetchMessages()
    } else if (confirmData.type === 'convo' && selectedUser) {
      const res = await deleteConversation(user.id, selectedUser.id)
      if (res.success) {
    ...
    <div className={`${styles.layoutWrapper} ${selectedUser ? styles.activeOnDetail : styles.activeOnList}`}>
      <aside className={styles.sidebar}>
        ...
        <div className={styles.chatList}>
          ...
          {users.length === 0 && (
             <div style={{ padding: '60px 20px', textAlign: 'center', opacity: 0.5, fontSize: '0.8rem' }}>
                Klik "+" untuk memulai percakapan.
             </div>
          )}
        </div>

        <button className={styles.fabNewChat} onClick={() => setIsModalOpen(true)} title="Chat Baru">
      ...
      <main className={styles.chatMain}>
        {selectedUser ? (
          <>
            ...
            <form className={styles.inputArea} onSubmit={handleSend}>
              <div className={styles.inputWrapper}>
                <input type="text" placeholder="Ketik pesan..." value={inputText} onChange={(e) => setInputText(e.target.value)} disabled={loading} />
              </div>
          ...
          <div className={styles.noSelection}>
            <h2>Pesan Saya</h2>
            <p>Pilih salah satu kontak untuk mulai berkomunikasi secara profesional.</p>
          </div>
        )}
      </main>
      ...
      {isModalOpen && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <div className={styles.modalHeader}><h3>Mulai Chat</h3><button onClick={() => setIsModalOpen(false)}>✕</button></div>
            <div className={styles.modalBody}>
              <div className={styles.modalOption} onClick={handleStartAdminChat}>
                <div className={styles.avatar}>A</div>
                <div><h4>Pusat Admin</h4><p>Langsung tanya ke admin pusat</p></div>
              </div>
              <div className={styles.modalDivider}>Atau Cari Rekan Kerja</div>
      ...
      {isConfirmOpen && (
        <div className={styles.modalOverlay} style={{ zIndex: 3000 }}>
          <div className={styles.confirmBox}>
            <h3>Hapus Obrolan?</h3>
            <p>{confirmData?.type === 'message' ? "Hapus pesan ini?" : "Hapus seluruh riwayat percakapan?"}</p>
